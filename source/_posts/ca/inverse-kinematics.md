...
在電腦動畫或機器人學中，我們可能會希望一個人物或機器人做到特定的動作。一個物件可能會有很多關節，假設每個關節都是 3 個維度 (DoF)，一個手臂假設是三段關節，那就會是 9 個維度，如果以正向動力學的方式去推敲怎樣達到我們想要的姿勢幾乎不可能。這時候就會採用反向動力學 (Inverse Kinematics, IK)，當我們給物件特定姿勢後，用反推的方式算出物件如何運動可以達到給定姿勢。
...
對於一個有多關節的剛體連結體，我們希望物體終端可以動到特定位置，那麼每個關節的初始位置 $\mathbf s_i$ 都會轉動，使得終端關節可以達到目標位置 $\mathbf t$。
...
透過反向動力法 (IK)，我們得以物件從原本狀態反推出如何動作達到我們想要的姿勢，對於電腦動畫而言，IK 可以讓動畫人物讓我們設定關鍵影格讓人物做到想要的動作；而對機器人學來說，IK 讓機器手臂可以順利抵達目標作業位置。
